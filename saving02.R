...
reop_ids     <- as.character(101:200) 
non_surg_ids <- as.character(201:250)
...
raw_data <- bind_rows(
  ...
  mutate(
    ...
    profile = case_when(
      # 1. 60% of Reoperated patients at Encounter 2 are 'Sick', 40% are 'Healthy'
      !is_index_surgery & reoperated_gen ~ if_else(runif(n()) < 0.6, "Sick", "Healthy"),
      
      # 2. 40% of ALL Index Surgeries are 'Sick'
      is_index_surgery & (runif(n()) < 0.4) ~ "Sick",
      
      # 3. 20% of Non-Surgical readmissions are 'Sick'
...
vitals_results <- master_measurements %>%
...
lab_results <- master_measurements %>%
...
vaso_results <- master_measurements %>%
...
fluid_results <- master_measurements %>%
...
raw_surgery_table <- raw_data

message("!!! SUCCESS: Encounter-level severity and timing bridge ready.")